...
export class ShellRunnerDetails extends Phaser.GameObjects.Container {
  scene: AbstractScene;

  overlay!: Phaser.GameObjects.Image;

  shellRunnerName!: Phaser.GameObjects.Text;
  ...
  private addTexts() {
    ...
    this.shellRunnerName = this.scene.add.text(0, this.overlay.getTopCenter().y - 36, 'Name of Shell Runner', titleConfig).setAlign('center').setOrigin(0.5, 0.5);
    this.shellRunnerName.setFontSize(48);
  ...
  updateShellRunnerDetails(details: IUserNftWithMetadata) {
    if (!details || !details.metadata) {
      this.shellRunnerName.text = 'No Shell Runner';
      this.id.text = '-';
      this.speedText.text = '0';
      this.ageText.text = '-';
      this.travelledText.text = '-';
      this.dobText.text = '-';
      return;
    }

    this.shellRunnerName.text = details.metadata.name ?? 'Shell Runner';
