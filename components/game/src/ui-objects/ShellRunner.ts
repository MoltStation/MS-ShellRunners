...
export class ShellRunner extends Phaser.GameObjects.Container {
  scene: AbstractScene;
  ...
  setupDisplayShellRunner(index: number) {
    if (!this.displayImage) {
      this.addDisplayImage(`shellRunner_display_${index}`);
    } else {
      this.displayImage.setTexture(`shellRunner_display_${index}`);
    }
  }

  setupGameShellRunner() {
    this.addLimbs();
    this.addTail();
    this.addHead();
    this.addEyes();
  ...
  changeGameShellRunner(index: number) {
    this.limbs.leftHand.setTexture(`shellRunner_left_hand_${index}`);
    this.limbs.leftFoot.setTexture(`shellRunner_left_foot_${index}`);
    this.limbs.rightHand.setTexture(`shellRunner_left_hand_${index}`);
    this.limbs.rightFoot.setTexture(`shellRunner_left_foot_${index}`);
    this.tail.setTexture(`shellRunner_tail_${index}`);
    this.head.setTexture(`shellRunner_head_${index}`);
    this.eyes.setTexture(`shellRunner_eyes_${index}`);
    this.innerShell.setTexture(`shellRunner_inner_shell_${index}`);
    this.outerShell.setTexture(`shellRunner_outer_shell_${index}`);
  }
  ...
  playPawnReviveTween(): void {
    this.scene.tweens.add({
      targets: this,
      scale: { from: 0, to: this.shellRunnerOrigScale, ease: TWEEN_EASING.EXPO_EASE_OUT, duration: 250 },
