...
export class GameManager {
  ...
  private addEventHandlers() {
    this.gameComponents.pawn.shellRunner.on(CUSTOM_EVENTS.PAWN_REVIVED, () => {
  ...
  checkCollision() {
    ...
    if (this.gameComponents.pawn.shellRunner.isGhost || this.gameComponents.pawn.shellRunner.isInvincible) {
      return false;
    }
  ...
  private getCollectedCount(collectibles: Array<Collectible>) {
    ...
    for (let i = 0; i < collectibles.length; ++i) {
      ...
      let dx = (pawn.shellRunner.x + PAWN_RADIUS) - (collectibles[i].x + COLLECTIBLE_RADIUS);
      let dy = (pawn.shellRunner.y + PAWN_RADIUS) - (collectibles[i].y + COLLECTIBLE_RADIUS);
      let distance = Math.sqrt(dx * dx + dy * dy);

      if (distance < PAWN_RADIUS + COLLECTIBLE_RADIUS) {
        // collision detected!
        collectibles[i].playConsumeTween(pawn.shellRunner.x, pawn.shellRunner.y);
  ...
  private getCollectedPowerUp(powerUps: Array<PowerUp>) {
    ...
    for (let i = 0; i < powerUps.length; ++i) {
      ...
      let dx = (pawn.shellRunner.x + PAWN_RADIUS) - (powerUps[i].x + POWER_UP_RADIUS);
      let dy = (pawn.shellRunner.y + PAWN_RADIUS) - (powerUps[i].y + POWER_UP_RADIUS);
      let distance = Math.sqrt(dx * dx + dy * dy);

      if (distance < PAWN_RADIUS + POWER_UP_RADIUS) {
        // collision detected!
        powerUps[i].playConsumeTween(pawn.shellRunner.x, pawn.shellRunner.y);
        return powerUps[i];
      }
    }
  }
  ...
  private checkForObstacleCollision(obstacles: Array<Obstacle>) {
    ...
    for (let i = 0; i < obstacles.length; ++i) {
      let testX = pawn.shellRunner.x;
      let testY = pawn.shellRunner.y;
      ...
      let distX = pawn.shellRunner.x - testX;
      let distY = pawn.shellRunner.y - testY;
