...
import { ShellRunnerSelectionMenu } from '../ui-objects/ShellRunnerSelectionMenu';
...
export class UIManager {
  ...
  shellRunnerSelectionMenu: ShellRunnerSelectionMenu;
  sideBar!: SideBar;
  coreUI!: CoreUI;
  ...
  private snapshotTimer?: Phaser.Time.TimerEvent;
  ...
  constructor(scene: AbstractScene, gameManager: GameManager) {
    ...
    this.shellRunnerSelectionMenu = new ShellRunnerSelectionMenu(this.scene).setDepth(DEPTH.ui);
  ...
  showShellRunnerSelection(initShellRunnersData: Array<IUserNftWithMetadata>) {
    const hasNfts = !!initShellRunnersData && initShellRunnersData.length > 0;
    const data = hasNfts
      ? initShellRunnersData
      : [
          {
            tokenId: 0,
            tokenUri: '',
            metadata: {
              name: 'Default Runner',
              description:
                'No ShellRunner NFTs found. Play with a default runner and mint after your run.',
              componentIndices: {
                eyes: '1',
                hands: '1',
                head: '1',
          ...
          } as IUserNftWithMetadata,
        ];

    this.overlayText.hide();
    this.shellRunnerSelectionMenu.populateShellRunners(data, 0);
    this.shellRunnerSelectionMenu.showMenu();
  }
  ...
  private handleGameOver() {
    ...
    this.resultScreen.updateResultDetails({
      travelled: this.distanceMeter.text,
      score: this.coreUI.scoreText.text,
      isMintable: score > highScore,
      highScore: this.scene.initGameData.highScore,
      hasShellRunnerNft: !!this.scene.initGameData.hasShellRunnerNft,
    })
  ...
  private addEventHandlers() {
    ...
    this.gameManager.gameComponents.pawn.shellRunner.on(CUSTOM_EVENTS.PAWN_DEAD, () => {
      this.coreUI.decrementLives();
      if (this.coreUI.lives === 0) {
        this.handleGameOver();
      } else {
        this.coreUI.hungerMeter.fillUpBar();
        this.gameManager.gameComponents.startShellRunnerRevival();
      }
    });
    ...
    this.menu.yesButton.on(CUSTOM_EVENTS.BUTTON_CLICKED, () => {
      if (this.scene.initGameData.exitToCoreCB) {
        this.scene.initGameData.exitToCoreCB();
        return;
      }
      this.menu.hideMenu();
      this.gameManager.gameComponents.resetCamera();
      // Ensure the scene isn't paused (pause pauses ALL tweens and time),
      // otherwise the UI can appear to freeze when switching to the selection menu.
      this.gameManager.handleGameResume();
      this.gameManager.endGame();
      this.stopSnapshotTimer();
      this.hideGameUI();
      ...
      this.showShellRunnerSelection(this.scene.initGameData.initMetaData);
    });
    ...
    this.shellRunnerSelectionMenu.on(CUSTOM_EVENTS.START_GAME, (speed: number, index: number) => {
      this.gameManager.gameComponents.pawn.changePawn(speed, index);
      this.gameManager.gameComponents.pawn.showPawnInitially();
    });
    this.gameManager.gameComponents.pawn.shellRunner.on(CUSTOM_EVENTS.PAWN_SPAWNED, () => {
      this.showGameUI();
      this.logo.hideLogo();
      this.gameManager.startGame();
      this.scene.initGameData.sessionStartCB?.();
      this.startSnapshotTimer();
    });
  }

  private startSnapshotTimer() {
    if (!this.scene.initGameData.snapshotScoreCB) {
      return;
    }
    if (this.snapshotTimer) {
      this.snapshotTimer.remove(false);
    }
    const delay = this.scene.initGameData.snapshotIntervalMs ?? 5000;
    this.snapshotTimer = this.scene.time.addEvent({
      ...
      callback: () => {
        ...
        this.scene.initGameData.snapshotScoreCB?.(this.coreUI.score);
      },
    });
  }
  ...
  resizeAndRepositionElements(): void {
    if (this.gameManager.currentResizeState === EResizeState.GAME) {
      ...
      this.shellRunnerSelectionMenu.resizeAndRepositionElements();
    }
  }
  ...
  update(delta: number): void {
    ...
    if (!this.gameManager.gameComponents.pawn.shellRunner.isGhost && !this.gameManager.gameComponents.pawn.shellRunner.isInvincible) {
