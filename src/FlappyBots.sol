...
interface IFlappyBotsAgentIdentity {
  function primaryIdentity(address owner) external view returns (uint256);
}

/// @title FlappyBots
/// @notice MoltStation game NFT contract for Flappy Bots.
contract FlappyBots is ERC721URIStorage, ReentrancyGuard, EIP712, Ownable {
  using Counters for Counters.Counter;

  bytes32 private constant MINT_TYPEHASH = keccak256(
    "MintFlappyBot(uint256 chainId,address verifyingContract,address wallet,uint256 score,bytes32 tokenURIHash,uint256 nonce,uint256 deadline)"
  );
  bytes32 private constant UPGRADE_TYPEHASH = keccak256(
    "UpgradeFlappyBot(uint256 chainId,address verifyingContract,address wallet,uint256 score,bytes32 tokenURIHash,uint256 tokenId,uint256 nonce,uint256 deadline)"
  );
  ...
  IFlappyBotsAgentIdentity public identityContract;
  ...
  event NewFlappyBotGenerated(uint256 tokenId);
  event FlappyBotUpgraded(uint256 tokenId);
  ...
  function setIdentityContractAddress(address _contractAddress) public onlyOwner {
    require(_contractAddress != address(0), "Invalid identity address");
    identityContract = IFlappyBotsAgentIdentity(_contractAddress);
  }
  ...
  modifier onlyIdentityHolder() {
    require(identityContract.primaryIdentity(msg.sender) != 0, "Identity required");
    _;
  }
  ...
  function generateFlappyBot(
    ...
    require(userAddressToTokenId[msg.sender] == 0, "FlappyBot already minted");
    require(_score > userAddressToHighScore[msg.sender], "Already minted at this score before");
    ...
    emit NewFlappyBotGenerated(_tokenID);
  }

  function upgradeFlappyBot(
    uint256 _score,
    string memory _tokenURI,
    uint256 _tokenId,
    uint256 _nonce,
    ...
    require(userAddressToTokenId[msg.sender] != 0, "No FlappyBot minted yet");
    require(userAddressToTokenId[msg.sender] == _tokenId, "Upgrade token must match user primary token");
    require(ownerOf(_tokenId) == msg.sender, "Not token owner");
    require(_score > userAddressToHighScore[msg.sender], "Already minted at this score before");

    setHighScore(_score, msg.sender);
    _setTokenURI(_tokenId, _tokenURI);
    emit FlappyBotUpgraded(_tokenId);
  }
  ...
  function setSignedWalletAddress(address _walletAddress) public onlyOwner {
  ...
  function getUserOwnedNFTs(address _user) public view returns (NFTItem[] memory) {
